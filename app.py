...
def generate_pdf_file(customer_name, records, filepath):
    ...
    # Lägg till bild om filen finns, annars textrubrik
    if os.path.exists(LOGO_FILE):
        logo_element = RLImage(LOGO_FILE, width=6 * cm, height=2.2 * cm)
    else:
        logo_element = Paragraph("<b>LASO INVEST AB</b>", title_style)

    header_data = [[
        logo_element,
        Paragraph(
            f"<b>FALTURAUNDERLAG</b><br/><b>Datum:</b> {date.today().strftime('%Y-%m-%d')}<br/><b>Org.nr/Pers.nr:</b> {clean_str(first_rec.get('Kund_OrgNr', ''))}",
            meta_style,
        ),
    ]]
    ...
    t = Table(
        table_data,
        colWidths=[2.5 * cm, 8.5 * cm, 2.2 * cm, 2.3 * cm, 2.5 * cm],
    )
    t.setStyle(
        TableStyle([
...
def generate_offert_pdf(offertnr, records, filepath):
    ...
    header_data = [[
        logo_element,
        Paragraph(
            f"<font size=14 color='#2B6CB0'><b>OFFERT</b></font><br/><b>Offertnummer:</b> {offertnr}<br/><b>Datum:</b> {first_rec['Offertdatum']}<br/><b>Giltig t.o.m:</b> {first_rec['Giltig_Tom']}",
            meta_style,
        ),
    ]]
...
with tabs[0]:
    ...
    with col_a:
        datum_val = st.date_input(
            "Datum", value=date.today(), key=f"datum_{fc}"
        )
    with col_b:
    ...
    if st.button("➕ Lägg till rad i underlag"):
        if val_art == "-- Välj artikel --":
            ...
            pris = float(
                art_row["ArtPris"].replace("kr", "").replace(" ", "")
            )
        ...
        if st.button(
            ...
            if not k_namn.strip():
                ...
                if not df_data.empty:
                    try:
                        start_id = int(
                            pd.to_numeric(df_data["ID"]).max() + 1
                        )
                    except Exception:
                ...
                st.success(
                    f"Underlaget för {k_namn} har sparats och formuläret har"
                    " rensats!"
                )
                st.rerun()
...
with tabs[1]:
    ...
    with sub_tab1:
        ...
        if st.session_state.temp_offert_items:
            ...
            off_df_temp["Totalt"] = (
                off_df_temp["Antal"] * off_df_temp["A_Pris"]
            )
            ...
            edited_off_df["Totalt"] = (
                edited_off_df["Antal"] * edited_off_df["A_Pris"]
            )
            ...
            if st.button(
                ...
                if not off_k_namn.strip():
                    ...
                    df_offert = pd.concat(
                        [df_offert, pd.DataFrame(new_off_rows)],
                        ignore_index=True,
                    )
                    ...
                    st.success(
                        f"Offert {offert_nr} till {off_k_namn} har sparats och"
                        " formuläret har rensats!"
                    )
                    st.rerun()
    ...
    with sub_tab2:
        if df_offert.empty:
            ...
            valda_offertnr = st.selectbox(
                "Välj Offert", options=df_offert["Offertnr"].unique()
            )
            offert_data = df_offert[
                df_offert["Offertnr"] == valda_offertnr
            ].copy()

            if not offert_data.empty:
                st.markdown(
                    f"### Offert: {valda_offertnr} –"
                    f" {offert_data.iloc[0]['Kund_Namn']}"
                )

                pdf_filename = f"Offert_{valda_offertnr}.pdf"
                if generate_offert_pdf(valda_offertnr, offert_data, pdf_filename):
                    with open(pdf_filename, "rb") as f:
                        st.download_button(
                            label=f"📥 Ladda ner PDF ({pdf_filename})",
                            data=f,
...
with tabs[2]:
    ...
    st.caption(
        "💡 *Här kan du ändra priser, lägga till nya artiklar eller ändra"
        " kategorier directly i tabellen. Glöm inte att klicka på Spara.*"
    )
    ...
    if st.button("💾 Spara ändringar i artikeldatabasen"):
...
with tabs[3]:
    ...
    if df_data.empty:
        ...
        if st.button("💾 Spara ändringar i underlag"):
            save_data(edited_df)
            st.success("Ändringarna i underlaget har sparats!")
            st.rerun()

# ==========================================
# FLIK 5: FAKTURAUNDERLAG
# ==========================================
with tabs[4]:
    st.subheader("📄 Skapa & Exportera Fakturaunderlag")

    if df_data.empty:
        st.info("Inga registrerade arbeten finns att skapa underlag för.")
    else:
        kunder = df_data["Kund_Namn"].unique()
        vald_kund = st.selectbox("Välj Kund", options=kunder)

        kund_data = df_data[df_data["Kund_Namn"] == vald_kund].copy()

        pdf_filename = f"Fakturaunderlag_{vald_kund.replace(' ', '_')}.pdf"

        if generate_pdf_file(vald_kund, kund_data, pdf_filename):
            with open(pdf_filename, "rb") as f:
                st.download_button(
                    label=f"📥 Ladda ner PDF for {vald_kund}",
                    data=f,
                    file_name=pdf_filename,
                    mime="application/pdf",
                    type="primary",
        ...
        st.dataframe(kund_data, use_container_width=True)
